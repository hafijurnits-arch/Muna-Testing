...
const BottomNav = () => {
    ...
    // Hide bottom nav on these pages (profile sub-pages keep their own header)
    const hiddenPaths = ['/login', '/vendor', '/admin', '/privacy-policy', '/daily-market', '/chat'];
    const isProfileSubPage = location.pathname.startsWith('/profile/');
    const shouldHide = hiddenPaths.some(p => location.pathname.startsWith(p)) || isProfileSubPage;
    ...
    const isProfile = path === profileLink || path === '/profile';
